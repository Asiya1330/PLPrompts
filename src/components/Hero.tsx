...
export default function Hero() {
  ...
  const handleSendToSeller = async (name) => {
    // router.push(`/prompt/${name}`);

    //adding to chats and move to chat-page
      ...
      <div className="flex flex-col justify-center items-center">
        ...
        <div className="flex flex-col bg-white rounded-lg px-3 py-4 max-w-[270px] h-fit justify-center items-center mt-4">
          <img
            ...
            alt="freePrompt"
          />
          <p className="text-black text-2xl font-bold text-center pt-4">
            {fourHighestScoredPrompt?.["PromptBase"]?.name}
          </p>
          <h2 className="text-black font-bold my-4">$0.00</h2>
          <Link href={{ pathname: `/prompt/${fourHighestScoredPrompt?.["PromptBase"]?.name}`, query: { amount: '0', promptId: fourHighestScoredPrompt?.['PromptBase']?._id } }}>
          <button
            onClick={() =>
              handleSendToSeller(fourHighestScoredPrompt?.["PromptBase"]?.name)
            }
            className='getPrompt bg-[#F3848A] my-6 py-3 px-12 rouned-full text-black
        ...
        <h2 className="text-center pt-10">
          <span className="gradient-text">
            Professionally hand-picked prompts for
          </span>{" "}
